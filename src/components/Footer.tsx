...
export default function Footer({ onNavigate, settings }: FooterProps) {
  const phone = settings.footer_phone_number ?? '5566999337454';
  const instagram = settings.footer_instagram_url ?? '#';
    ...
    <footer className="bg-[#050505] border-t border-[#1a1a1a] mt-20">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10">
          <div className="lg:col-span-2">
            <div className="flex items-center gap-3 mb-4">
              ...
              />
              <div>
                <span className="text-white font-bold text-xl leading-none block">{settings.footer_company_name}</span>
                <span className="text-[#00ff88] text-xs tracking-widest uppercase">{settings.footer_tagline}</span>
              </div>
            </div>
          ...
          <div>
            ...
            <ul className="space-y-3">
              <li className="flex items-start gap-2.5">
                ...
                <a href={`tel:+${phone}`} className="text-gray-500 hover:text-[#00ff88] text-sm transition-colors duration-200">
              ...
              <li className="flex items-start gap-2.5">
                ...
                <span className="text-gray-500 text-sm">{settings.footer_email}</span>
              </li>
              <li className="flex items-start gap-2.5">
                <MapPin className="w-4 h-4 text-[#00ff88] mt-0.5 flex-shrink-0" />
                <span className="text-gray-500 text-sm">{settings.footer_location}</span>
              </li>
            </ul>
          </div>
        </div>
        ...
        <div className="mt-12 pt-6 border-t border-[#1a1a1a] flex flex-col sm:flex-row items-center justify-between gap-4">
          <p className="text-gray-600 text-xs">{settings.footer_copyright}</p>
          <p className="text-gray-700 text-xs">{settings.footer_tagline_bottom}</p>
        </div>
      </div>
    </footer>
  );
