...
export default function Hero({ onNavigate, settings }: HeroProps) {
  ...
  const techMsg = encodeURIComponent(settings.whatsapp_tech_message ?? 'Olá! Preciso de assistência técnica para meu iPhone.');
    ...
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden bg-[#050505]">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-32 text-center">
        <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-[#00ff88]/10 border border-[#00ff88]/20 mb-8">
          ...
          <span className="text-[#00ff88] text-sm font-medium tracking-wide">{settings.hero_badge_text}</span>
        </div>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-4 mb-6">
          ...
          >
            {settings.hero_btn_secondary}
          </button>
        </div>

        <div className="flex justify-center mb-20">
          <a
            href={`https://wa.me/${phone}?text=${techMsg}`}
            target="_blank"
          ...
          >
            ...
            {settings.hero_btn_whatsapp}
          </a>
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 max-w-3xl mx-auto">
          {[
            { icon: Shield, title: settings.hero_feature_1_title, desc: settings.hero_feature_1_desc },
            { icon: Zap, title: settings.hero_feature_2_title, desc: settings.hero_feature_2_desc },
            { icon: Star, title: settings.hero_feature_3_title, desc: settings.hero_feature_3_desc },
